...
async function fetchData() {
  const fetchURL = apiUrlBuilder('collections/pages/entries', { "filter[slug]": 'story-maps' });
...
async function fetchStoryMaps() {
  const fetchURL = apiUrlBuilder('collections/story_maps/entries', { "sort": '-date' });
...
export default async function StoryMapsPage() {
  const { data, error } = await fetchData();
  const storyMaps = await fetchStoryMaps();

  console.log('Story maps fetched:', storyMaps);

  console.log('Fetched page data:', data, error);
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-grow container mx-auto px-4 py-16">
          <h1 className="text-3xl font-bold mb-4">Story Maps</h1>
          <p className="text-red-600">Error loading page data: {error}</p>
        </main>
      </div>
    );
  }
        ...
        <div className="container mx-auto px-6">
          <div className="max-w-3xl">
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 text-slate-50">
              {data?.title || "Story Maps"}
            </h1>
            <div className="h-1 w-20 bg-accent mb-6 rounded-full"></div>
            <p className="text-lg md:text-xl text-slate-50/90 max-w-2xl">
              {data?.hero_description || "Explore interactive narratives that combine maps, images, and text to tell the stories of First American land allotments."}
            </p>
          </div>
        </div>
      </header>
