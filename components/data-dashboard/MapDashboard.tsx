...
export default function MapDashboard() {
  ...
  const handleApplyFilters = () => {
    console.log(
      `Applying filters: Tribe=${selectedTribe}, LandType=${selectedLandType}, Years=${timeRangeStart}-${timeRangeEnd}`
    );
    ...
    const filtered: GeoJSON.FeatureCollection = {
      ...
      features: geoJSONdata.features.filter((feature) => {
        ...
        const tribeMatch =
          selectedTribe === "all" || properties.tribe === selectedTribe;
        const landTypeMatch =
          selectedLandType === "all" ||
          properties.landType === selectedLandType;
        const yearMatch =
          properties.year >= timeRangeStart && properties.year <= timeRangeEnd;
                  ...
                  <div className="flex space-x-4 items-center">
                    <input
                      ...
                      onChange={(e) => {
                        const value = Math.max(
                          1830,
                          Math.min(timeRangeEnd, Number(e.target.value))
                        );
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const value = Math.max(
                          timeRangeStart,
                          Math.min(1930, Number(e.target.value))
                        );
          ...
          <div className="p-4 bg-white border-t">
            <div className="text-sm text-teal/80">
              <p>
                ...
                {selectedTribe === "all"
                  ? " all tribes"
                  : ` the ${selectedTribe} tribe`}
                ,
                {selectedLandType === "all"
                  ? " all land types"
                  : ` ${selectedLandType} lands`}
